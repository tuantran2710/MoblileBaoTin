...
class StatStoreTopProduct extends StatefulWidget {
  const StatStoreTopProduct({required this.tokenLogin, required this.storeSubId});
  final String tokenLogin;
  final int storeSubId;

  @override
  _StatStoreTopProductState createState() => _StatStoreTopProductState();
}

class _StatStoreTopProductState extends State<StatStoreTopProduct> {
  ...
  final String StatStoreTopProductUrl = "Product/GetListStatProductTopTree?token=";
  final String StatStoreTopProductKgUrl = "Product/GetListStatProductTopKg?token=";
  ...
  void fetchStatStoreTopProduct(int storeId, int year, int top) async {
    isUpdateUI = false;
    _ValueData data;
    lstProductTopTree.clear();
    lstProductTopKg.clear();

    var result = await http.get(Uri.parse(url + StatStoreTopProductUrl + tokenLogin + "&storeSubId=" + storeSubId.toString() +
        "&storeId=" + storeId.toString() + "&time=" + year.toString() + "&top=" + top.toString() + "&isYear=true&isTop=true"));
    ...
    result = await http.get(Uri.parse(url + StatStoreTopProductKgUrl + tokenLogin + "&storeSubId=" + storeSubId.toString() +
        "&storeId=" + storeId.toString() + "&time=" + year.toString() + "&top=" + top.toString() + "&isYear=true&isTop=true"));
  ...
  Future<List<String>> fetchActiveStores() async {
    ...
    setState(() {
      if (lstResult.length>0) {
        ...
        fetchStatStoreTopProduct(_storeId(dropdownValue), _year, _top);
      }
    });
    return lstResult;
  }
             ...
             children: <Widget>[
               ...
               Column(
                 children: <Widget>[
                   SfCircularChart(
                     // Chart title
                       title: ChartTitle(text: 'THỐNG KÊ S.L'),
                       // Enable legend
                   ...
                   SfCircularChart(
                     // Chart title
                       title: ChartTitle(text: 'THỐNG KÊ K.L'),
                       // Enable legend
                       legend: Legend(isVisible: true),
                       // Enable tooltip
                       tooltipBehavior: TooltipBehavior(enable: true),

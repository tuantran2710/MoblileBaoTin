...
class PaidReport extends StatefulWidget {
  const PaidReport({required this.tokenLogin, required this.income});
...
class _PaidReportState extends State<PaidReport> {
  ...
  final String listUrl = "GetPaids?token=";
  ...
  void fetchTickets() async {
    isUpdateUI = false;
    var result = await http.get(Uri.parse(url + listUrl + tokenLogin + "&isIncome=" + (income?"true":"false")));

    List<dynamic> lst = json.decode(result.body)['paidValues'];
    _Data data;
    for(dynamic item in lst)
      {
          data = _Data(item['CustomerCode'], item['Customer'], item['PaymentType'], '');
                      ...
                      TextFormField(
                        ...
                        onChanged: (value) {
                          if (value.length == 0) {
                            ...
                            for (dynamic item in lstSource) {
                              if (item.customerCode.toString().contains(value) || item.customer.toString().contains(value))
                                lstPayment.add(item);
                            }
...
class _Data {
  _Data(this.customerCode, this.customer, this.paymentType, this.data);
  setCustomer(dynamic payments) {
    customer = payments['CustomerCode'] + ' - ' + payments['Customer'];
  }
  ...
  setData(dynamic payments) {
    data = payments['PaymentType'] + '\r\n' +
        'Thành Tiền: ' +
        format(payments['Total'].toString()) +
    'đ\r\nGiảm Tiền: ' +
        format(payments['Decrease'].toString()) + 'đ';
  }
  String customerCode;
  String customer;
  String paymentType;

...
class WarningRemain extends StatefulWidget {
  const WarningRemain({required this.tokenLogin, required this.storeId, required this.storeSubId, required this.isMax});
  ...
  @override
  _WarningRemainState createState() => _WarningRemainState();
}

class _WarningRemainState extends State<WarningRemain> {
  String tokenLogin = "";
  final String url = "http://125.253.121.180/CommonService.svc/Store/";
  final String warningListUrl = "GetWarningStoreRemain?token=";
  int storeId = 0;
  int storeSubId = 0;
  bool isMax = false;
  ...
  void fetchInventory() async {
    ...
    List<dynamic> lst = json.decode(result.body)['warningstoreremains'];
    _Data data;
...
class _Data {
  ...
  setData(dynamic remainStores) {
    data = 'K.L Tồn: ' +
        format(remainStores['Remain'].toString()) + '\r\nK.L Cảnh Báo: ' + format(remainStores['WarningKg'].toString()) + '\r\nK.L Cần Thêm: ' +
        format(remainStores['NeedKg'].toString());
  }

...
class WarningExpired extends StatefulWidget {
  const WarningExpired({required this.tokenLogin, required this.storeId, required this.storeSubId});
  ...
  @override
  _WarningExpiredState createState() => _WarningExpiredState();
}

class _WarningExpiredState extends State<WarningExpired> {
  String tokenLogin = "";
  final String url = "http://125.253.121.180/CommonService.svc/Store/";
  final String warningListUrl = "GetWarningStoreExpired?token=";
  int storeId = 0;
  int storeSubId = 0;
  ...
  void fetchInventory() async {
    isUpdateUI = false;
    var result = await http.get(Uri.parse(url + warningListUrl + tokenLogin + "&storeSubId=" + storeSubId.toString() + "&storeId=" + storeId.toString() +
    "&beforeDays=" + beforeDays.toString())).timeout(
    ...
    List<dynamic> lst = json.decode(result.body)['warningstoreexpireds'];
    _Data data;
...
class _Data {
  ...
  setData(dynamic remainStores) {
    data = 'NCC: ' + remainStores['SupplierCode'].toString() + ' - ' + remainStores['Supplier'].toString() +
        ...
        '\r\nNgày Hết Hạn: ' + remainStores['strExpiredDate'].toString() +
        '\r\nSố Ngày Đã Hết Hạn: ' + remainStores['ExpiredDays'].toString();
  }
